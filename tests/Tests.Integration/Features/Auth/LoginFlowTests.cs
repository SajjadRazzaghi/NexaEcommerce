...
using NexaECommerce.Server.Features.Auth;
using NexaECommerce.Tests.Integration.Fixtures;
using Shouldly;

namespace NexaECommerce.Tests.Integration.Features.Auth;
...
public sealed class LoginFlowTests(CustomWebApplicationFactory factory)
{
    ...
    public async Task Unknown_email_also_returns_invalid_credentials_not_a_distinct_code()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/auth/login",
            new LoginRequest("nobody@nexaecommerce.test", "whatever-Pass1"));
