# NexaECommerce — User Guide

A practical, feature-by-feature guide to what NexaECommerce gives you and how to run it. For copy-pasteable how-tos see [RECIPES.md](RECIPES.md); for the one-screen reference see [CONVENTIONS.md](CONVENTIONS.md); for AI-agent guidance see [../CLAUDE.md](../CLAUDE.md).

**Edition note:** this guide covers the full NexaECommerce platform. Depending on the edition and options this project was scaffolded with, some features below may not be present — the full online docs — which mark each feature Community or Pro — are at <https://docs.nexaecommerce.ebenmonney.com>.

---

## 1. What NexaECommerce is

NexaECommerce is an opinionated starter template for **ASP.NET Core 10 + React 19** line-of-business apps.
...
```bash
dotnet run --project NexaECommerce.Server
```
...
All settings live in `NexaECommerce.Server/appsettings.json`, with `appsettings.Development.json` overriding
...
|---|---|---|
| `ConnectionStrings:Default` | SQLite `App_Data/nexaecommerce.db` | EF Core connection string. Must match `Database:Provider`. |
| `Database:Provider` | `sqlite` | Active EF Core provider: `sqlite`, `postgres`, or `sqlserver`. See [§3.2](#32-database-providers). |
| `App:ProductName` | `NexaECommerce` | Branding shown in emails and the UI. |
...
The model is **provider-agnostic** — NexaECommerce runs on SQLite, PostgreSQL, or SQL Server. The provider is
...
but carries a real **parity risk**: SQLite stores dates and money differently (NexaECommerce applies
...
By default (no `Email:Smtp:Host` / `Email:FromAddress`) NexaECommerce uses a **console email sender**: every
...
- **CLI wizard** (easiest) — `dotnet tool install -g NexaECommerce.Cli`, then `nexaecommerce new` walks you through edition, database, features, OAuth, and theme, resolves feature dependencies, and runs `dotnet new` for you. Scriptable too: `nexaecommerce new -n MyApp --tier basic --with dashboard,webhooks --yes` (or `--dry-run`).
- **Raw template** — the free Basic edition is `dotnet new install NexaECommerce.Templates`, then `dotnet new nexaecommerce -n MyApp`. The full Pro template (`NexaECommerce.Pro.Templates`, with the `--tier`/feature/database flags below) installs from the sponsor-gated feed (see the configurator).
- **Online configurator** — pick features visually at <https://nexaecommerce.ebenmonney.com> and download a ZIP.

The options map 1:1 across all three:

| CLI flag | Configurator | Values | Default | Effect |
|---|---|---|---|---|
| `-n, --name` | Project name | identifier | `NexaECommerce` | Names the solution, namespaces, and folders. |
...
the **events** it should receive — and NexaECommerce POSTs a signed JSON body there whenever one of those
...
**Verifying a delivery:** each request carries `X-NexaECommerce-Event`, `X-NexaECommerce-Delivery`,
`X-NexaECommerce-Timestamp`, and `X-NexaECommerce-Signature: sha256=<hex>` — an HMAC-SHA256 of the raw body using
...
feature looks built on NexaECommerce's primitives. It's gated by per-area permissions (`orders.*`,
...
NexaECommerce ships with always-on multi-tenancy plumbing that stays **invisible until you turn it on** —
...
- **Install as an app (PWA)** — on a supported browser, NexaECommerce offers an **Install** banner; once
...
| Task | How |
|---|---|
| **Run everything** | `dotnet run --project NexaECommerce.Server` (auto-starts the client). |
| **Run the tests** | `dotnet test NexaECommerce.slnx` (unit + integration). Unit only: `dotnet test tests/Tests.Unit`. |
| **Migrations** | Auto-applied on dev boot. Add one: `dotnet ef migrations add <Name> --project NexaECommerce.Server`. |
...
| **Scaffold a fresh app from this template** | `dotnet new nexaecommerce -n MyApp` — toggles: `--tier` (`pro`/`basic`), `--IncludeDemo`, `--Google`/`--Microsoft`/`--GitHub`. See the [configurator reference](https://docs.nexaecommerce.ebenmonney.com/reference/configurator). |
...
- **[docs.nexaecommerce.ebenmonney.com](https://docs.nexaecommerce.ebenmonney.com)** — the full online documentation, with a tier-aware feature reference and the editions breakdown.
