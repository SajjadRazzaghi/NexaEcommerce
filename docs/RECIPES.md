...
**Edition note:** some recipes cover optional subsystems (search, webhooks, the dashboard, export, multi-tenancy) that a trimmed/Community scaffold may not include. The full online docs — which mark each feature Community or Pro — are at <https://docs.nexaecommerce.ebenmonney.com>.
...
```bash
cp -r NexaECommerce.Server/Features/_Template NexaECommerce.Server/Features/Projects
```
...
```csharp
namespace NexaECommerce.Server.Features.Projects;
...
```bash
dotnet ef migrations add AddProjects --project NexaECommerce.Server
```
...
```bash
cp -r nexaecommerce.client/src/pages/_template "nexaecommerce.client/src/pages/(app)/projects"
```
...
```csharp
using NexaECommerce.Server.Platform.Search;
