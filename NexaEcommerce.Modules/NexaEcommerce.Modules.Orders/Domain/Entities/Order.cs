...
public sealed class Order : AggregateRoot
{
        ...
        string? shippingPostalCode)
    {
        ...
        if (shippingAmount < 0)
            throw new ArgumentOutOfRangeException(nameof(shippingAmount));

        if (discountAmount < 0)
            throw new ArgumentOutOfRangeException(nameof(discountAmount));

        if (string.IsNullOrWhiteSpace(shippingFullName))
            throw new ArgumentException(nameof(shippingFullName));

        if (string.IsNullOrWhiteSpace(shippingPhone))
            throw new ArgumentException(nameof(shippingPhone));

        if (string.IsNullOrWhiteSpace(shippingAddress))
            throw new ArgumentException(nameof(shippingAddress));

        if (string.IsNullOrWhiteSpace(shippingCity))
            throw new ArgumentException(nameof(shippingCity));
