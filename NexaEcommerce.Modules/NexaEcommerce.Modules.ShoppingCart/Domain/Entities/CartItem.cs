...
public sealed class CartItem : BaseEntity
{
    ...
    internal void IncreaseQuantity(int quantity, decimal unitPrice, string productName, string? imageUrl)
    {
        if (quantity <= 0) throw new ArgumentOutOfRangeException(nameof(quantity));
        Quantity += quantity;
    ...
    internal void SetQuantity(int quantity, decimal unitPrice, string productName, string? imageUrl)
    {
        if (quantity <= 0) throw new ArgumentOutOfRangeException(nameof(quantity));
        Quantity = quantity;
        UpdateSnapshot(unitPrice, productName, imageUrl);
    ...
    private void UpdateSnapshot(decimal unitPrice, string productName, string? imageUrl)
    {
        if (unitPrice < 0) throw new ArgumentOutOfRangeException(nameof(unitPrice));
        if (string.IsNullOrWhiteSpace(productName)) throw new ArgumentException("Product name is required.", nameof(productName));
