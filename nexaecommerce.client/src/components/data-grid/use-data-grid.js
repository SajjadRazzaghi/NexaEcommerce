...
export function useDataGrid(options) {
    ...
    for (const [key, value] of params.entries())
        if (!RESERVED.has(key) && value)
            filters[key] = value;
    const isDefaultSort = (s) => !!defaultSort && !!s && s.id === defaultSort.id && s.desc === (defaultSort.desc ?? false);
    // --- setters: each rewrites the URL (replace, so we don't spam history) and resets to page 1 ---
    const update = (mutate) => setParams((prev) => {
        const next = new URLSearchParams(prev);
        mutate(next);
        return next;
    }, { replace: true });
    const setPage = (value) => update((p) => {
        if (value <= 1)
            p.delete('page');
        else
            p.set('page', String(value));
    });
    const setPageSize = (value) => update((p) => {
        if (value === initialPageSize)
            p.delete('pageSize');
        else
    ...
    const setSearch = (value) => update((p) => {
        if (value.trim())
            p.set('search', value);
        else
            p.delete('search');
    ...
    const setFilter = (field, value) => update((p) => {
        if (value === null || value === '')
            p.delete(field);
        else
            p.set(field, value);
    ...
    const reset = (state) => {
        ...
        if (state) {
            ...
            if (state.pageSize && state.pageSize !== initialPageSize)
    ...
    const apiQuery = () => {
        ...
        if (sorting[0])
            p.set('sort', `${sorting[0].id}:${sorting[0].desc ? 'desc' : 'asc'}`);
        if (search.trim())
    ...
    return {
        ...
        reset,
        activeFilterCount,
        state: { page, pageSize, sorting, search, filters },
    };
}
